...
# Generate travis APIv3 request to trigger corresponding systemtests based on the adapter type. 
# To adjust it for newly added system test modify dictionaries below with the
# name of the adapter repository, and systemtests that are ran for it
...
def generate_travis_job(adapter, user): 
  ...
  job_templates= {
      ...
      "after_failure": Template('python push.py -t $TEST; python triger_systemtests.py --failure --owner $USER --adapter $ADAPTER') }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--adapter', type=str, help="Choose adapter for which you want to trigger systemtests", required=True, choices = ["openfoam", "su2", "calculix"])
    parser.add_argument('--failure', help="Whether you want to send failure status to origin adapter travis", action="store_true")
    ...
    if args.failure:
      ...
      trigger_travis_build( generate_failure_callback(), args.owner,
          repo)
       
    else: 
      trigger_travis_build( generate_travis_job(args.adapter, args.owner), args.owner,
          'systemtests' )
